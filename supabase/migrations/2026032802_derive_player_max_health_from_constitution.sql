...
begin
    ...
    case action
        ...
        when 'reallocate-stats' then
            respec_cost := round((coalesce((save_payload->>'money')::int, 0) / 2.0)::numeric)::int;
            if active_raid = 'null'::jsonb and coalesce((save_payload->>'money')::int, 0) >= respec_cost then
                ...
                save_payload := jsonb_set(save_payload, '{money}', to_jsonb(coalesce((save_payload->>'money')::int, 0) - respec_cost), true);
                save_payload := jsonb_set(save_payload, '{draftStats}', normalized_draft_stats, true);
    ...
    save_payload := jsonb_build_object(
        ...
        'playerMaxHealth', 10 + (2 * coalesce((coalesce(save_payload->'acceptedStats', accepted_stats)->>'constitution')::int, 8)),
