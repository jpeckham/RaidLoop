create or replace function game.raid_medkit_count(items jsonb, medkits int default 0)
returns int
language sql
stable
as $function$
    select greatest(coalesce(medkits, 0), 0)
        + coalesce(
            (
                select count(*)
                from jsonb_array_elements(coalesce(items, '[]'::jsonb)) value
                where game.payload_item_name(value) = 'Medkit'
            ),
            0
        );
$function$;

create or replace function game.jsonb_array_remove(items jsonb, index_to_remove bigint)
returns jsonb
language sql
stable
as $function$
    select coalesce(
        (
            ...
            where ordinality <> index_to_remove + 1
        ),
        '[]'::jsonb
    );
$function$;

create or replace function game.raid_strip_medkits(items jsonb)
returns jsonb
language sql
stable
as $function$
    select coalesce(
        (
            ...
            from jsonb_array_elements(coalesce(items, '[]'::jsonb)) value
            where game.payload_item_name(value) is distinct from 'Medkit'
        ),
        '[]'::jsonb
    );
$function$;
...
declare
    ...
    raw_log_entries jsonb := coalesce(payload->'logEntries', payload->'LogEntries');
    normalized_carried_loot jsonb := game.normalize_items(coalesce(payload->'carriedLoot', payload->'CarriedLoot'));
    normalized_medkits int := game.raid_medkit_count(
        game.normalize_items(coalesce(payload->'carriedLoot', payload->'CarriedLoot')),
        greatest(coalesce((payload->>'medkits')::int, 0), 0)
    );
begin
    normalized_carried_loot := game.raid_strip_medkits(normalized_carried_loot);
    ...
    normalized_payload := jsonb_set(
        normalized_payload,
        '{carriedLoot}',
        normalized_carried_loot,
        true
    );
    normalized_payload := jsonb_set(
        normalized_payload,
        '{medkits}',
        to_jsonb(normalized_medkits),
        true
    );
    normalized_payload := jsonb_set(
        normalized_payload,
    ...
    normalized_payload := jsonb_set(
        ...
        '{logEntries}',
        case
            when jsonb_typeof(raw_log_entries) = 'array' then raw_log_entries
            else '[]'::jsonb
        end,
        true
    );
...
begin
    ...
    medkits := game.raid_medkit_count(
        carried_loot,
        greatest(coalesce((raid_payload->>'medkits')::int, 0), 0)
    );
    carried_loot := game.raid_strip_medkits(carried_loot);
        ...
        if selected_item is not null then
            ...
            if slot_type in (0, 1, 2) then
                ...
                while game.raid_current_slots(carried_loot) > backpack_capacity and jsonb_array_length(carried_loot) > 0 loop
                    ...
                    discovered_loot := discovered_loot || jsonb_build_array(dropped_item);
                end loop;
    ...
    raid_payload := jsonb_set(raid_payload, '{carriedLoot}', game.raid_strip_medkits(game.normalize_items(coalesce(raid_payload->'carriedLoot', carried_loot))), true);
    ...
    raid_payload := jsonb_set(
        raid_payload,
        '{medkits}',
        to_jsonb(game.raid_medkit_count(
            game.normalize_items(coalesce(raid_payload->'carriedLoot', carried_loot)),
            greatest(coalesce((raid_payload->>'medkits')::int, medkits), 0)
        )),
        true
    );
    ...
    raid_payload := jsonb_set(raid_payload, '{lootSlots}', to_jsonb(game.raid_current_slots(coalesce(raid_payload->'carriedLoot', '[]'::jsonb))), true);
    raid_payload := jsonb_set(raid_payload, '{challenge}', to_jsonb(challenge), true);
    raid_payload := jsonb_set(raid_payload, '{distanceFromExtract}', to_jsonb(distance_from_extract), true);

    current_encumbrance := game.current_encumbrance(
        coalesce(raid_payload->'equippedItems', equipped_items) || coalesce(raid_payload->'carriedLoot', '[]'::jsonb),
...
with normalized_game_saves as (
    select
        saves.user_id,
        case
            when saves.payload ? 'activeRaid' and jsonb_typeof(saves.payload->'activeRaid') = 'object'
                then jsonb_set(
                    saves.payload,
                    '{activeRaid}',
                    game.normalize_active_raid_payload(
                        saves.payload->'activeRaid',
...
revoke all on function game.raid_medkit_count(jsonb, int) from public;
revoke all on function game.raid_strip_medkits(jsonb) from public;
